...
The local/mock build now includes an executable synthetic provider profile. It requires no provider-side retention, no telemetry, no abuse logging, prohibited secondary use, prohibited training use and prohibited tool execution. The profile id/version/digest are preserved in the run context and audit envelope. This does not approve a production provider profile or provider-side retention/telemetry contract.

Provider unavailability is now an executable fail-closed path in the local/mock pipeline. Runtime provider failure produces no released AI text, keeps the deterministic canonical fallback available and records only the stable `PROVIDER_UNAVAILABLE` violation code in the minimized audit envelope. Provider exception text and diagnostics are not retained.

The deterministic fallback is now represented as `MarketOptionCanonicalExplanation`, a typed immutable application value object rather than an arbitrary mapping. It preserves option state, reversibility, Policy/version, `reference_time`, `knowledge_cutoff` and result boundary, and exposes an explicit mapping projection only for canonical audit digesting.

The local/mock pipeline now makes classification inheritance executable without adding production persistence. `MarketOptionCanonicalExplanation` and `MarketOptionExplanationPromptPayload` carry a protected derived-output classification and disclosure restrictions declaring that derived knowledge inherits source restrictions, generation never declassifies information and AI explanation output is not reusable for export inference or redistribution.

On 2026-09-05, ADR-0075 and `docs/specs/proposed/2026-09-05-ai-explanation-production-decision-package.md` were accepted with changes as the production decision package for ADR-0074. They define ProviderProfile, DataContract and minimized audit storage boundaries with additional hardening, but do not approve a concrete provider contract, production adapter, API/UI behavior, migration, raw prompt/output retention or buyer-facing aggregate AI explanation.
