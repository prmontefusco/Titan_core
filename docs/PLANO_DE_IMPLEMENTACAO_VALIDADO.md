...
**Status:** em execução — Passo 4.4 concluído e aprovado; próximo incremento: Passo 4.5
...
**Resultado:** concluído e aprovado. A ADR 0031 define Permission canônica no catálogo da Organization operadora, Role por Organization, atribuição temporal à Membership e revogação append-only. Constraints, RLS e testes comprovam resolução efetiva, remoção sem apagar histórico, isolamento e ausência de Permission direta ao User.
...
**Resultado:** concluído e aprovado. O checkpoint ancora a cadeia completa por agregado, preserva conjunto ordenado, delimitadores, contagem, cabeça, perfis e Digest; o verificador independente detecta omissão, adulteração e escopo divergente sem Merkle ou timestamp.
...
Os Passos 0.1 a 4.4 estão concluídos e aprovados. O próximo incremento é o **Passo 4.5 — Correção sem sobrescrita**.
