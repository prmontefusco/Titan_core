...
F7 was completed on 2026-09-04 as an application-only deterministic explanation guard. It prepares canonical explanation context from `MarketOptionAssessment` and validates explanation drafts against canonical Decision/Evaluation/Policy references, option state, reason codes, missing evidence types and limitations. It does not authorize an LLM provider integration, prompt execution, user-visible AI output, persistence, API, UI, worker, forecast, AI-created facts/gaps/rules/states, or changes to Evaluation/Decision/Dossier/VerificationBundle semantics.
