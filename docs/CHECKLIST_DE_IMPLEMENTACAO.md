...
**ADR-0062 — proposta detalhada para T-05B em 13 de agosto de 2026.** A investigação confirmou que os eventos append-only `livestock.identifier_attached` e `livestock.identifier_deactivated` já preservam o mínimo necessário para um leitor temporal: payload canônico versionado, `occurred_at`, `recorded_at`, Organization, agregado Animal, ordenação por versão e cadeia de integridade. A proposta limita o próximo corte a um leitor puro do Livestock Application sobre `DomainEventReader`: filtra por validade e conhecimento, interpreta exclusivamente schemas conhecidos e falha fechada em payload, versão ou lifecycle conflitante. A tabela/projeção atual `animal_identifiers` não será fonte de passado. **Estado:** desenho registrado na ADR-0062; implementação depende de aceite específico do corte, pois introduz a primeira interpretação semântica de payloads de eventos pelo Livestock.
