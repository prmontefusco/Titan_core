...
**F3.5 POLICY GATE — proposta ADR-0072 para histórico de audit/differencing:** criada `docs/adr/0072-market-supply-audit-history-visibility-for-differencing.md` como proposta para resolver a pendência normativa residual do F-09. A recomendação é manter raw `MarketSupplyQueryAuditRecord` owner-only por RLS e fazer a avaliação de differencing por pipeline de aplicação owner/contributor-scoped, retornando ao comprador somente a projeção pública uniforme. `docs/plans/CUT_F3_5_MARKET_SUPPLY_RELEASE_GATE_PACKAGE.md` foi atualizado para depender da ADR-0072 antes de F3.5, incluindo gates automatizados para buyer não ler raw audit, histórico obrigatório falhar fechado e differencing funcionar sem visibilidade bruta do comprador. Não houve código produtivo, migration, API, endpoint, permission seed, query cross-tenant produtiva, forecast ou CommercialDemand persistido.
