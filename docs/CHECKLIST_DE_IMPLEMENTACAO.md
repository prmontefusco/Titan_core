...
**P0 implementado — harness de integração sob role runtime em 13 de agosto de 2026; portão de banco pendente.** A fixture administrativa continua semeando e revertendo cada cenário sem commit, mas toda requisição HTTP executa agora por `SET LOCAL ROLE titan_app` (ou role configurada), a mesma identidade `NOSUPERUSER` e `NOBYPASSRLS` provisionada para o runtime. A role efêmera com grants próprios foi removida: se a credencial de runtime não existir ou possuir bypass, a fixture falha antes do teste. O CI aplica migrations e provisiona essa role antes da suíte, preservando a separação entre credencial administrativa e caminho da API. **Portão pendente:** testes de API com RLS e o conjunto amplo em banco novo devem passar sob essa configuração; o Docker Desktop local estava indisponível nesta execução, portanto não há declaração prematura de conclusão.
