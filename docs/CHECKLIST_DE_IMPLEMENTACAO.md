...
**Quarto corte concluído em 12 de agosto de 2026.** A ADR-0056 foi aceita com `MedicationSanitaryClassificationAssertion` específica do Livestock, preservando `NO_ASSERTION != UNKNOWN != DOES_NOT_APPLY` e sem generalização prematura. Assertions bitemporais de `ANTIMICROBIAL` possuem persistência append-only com RLS, API auditável e seleção por `reference_time`/`knowledge_cutoff`. `SANITARY_TEST_A_v1` compõe `medication_classification` como dimensão independente de `treatment_history`: qualquer tratamento material sem classificação suficiente mantém `INDETERMINADA`; classificação `APPLIES` admissível permite detectar tratamento antimicrobiano. O roteiro `apps/validacao/classificacao_sanitaria_medicamento.py` prova ausência distinta de UNKNOWN sem copiar identificadores e suporta `--pausar`. **Portão verificado:** 1232 testes PostgreSQL incluídos passaram; Ruff, format check, Mypy e Alembic check verdes.
