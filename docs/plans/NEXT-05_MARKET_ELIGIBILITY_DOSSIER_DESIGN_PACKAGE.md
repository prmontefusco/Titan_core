...
**CORTE 2 CONCLUÍDO EM 12 DE AGOSTO DE 2026.** `MarketEligibilityDossierTemplate` conecta exclusivamente `MARKET_TEST_A` ao `DossierService.build_and_store()` já existente. A persistência continua sendo a do `Dossier` Core, sem tabela, migration ou repositório de mercado; o template recusa outro profile e profile não sintético antes de construir o documento. Dois testes adicionais provam que o Dossier é guardado pelo repositório configurado, continua verificável pelo hash canônico e não permite `MARKET_TEST_B`. Não há endpoint, mudança de contrato público, mercado real, integração externa, `DecisionAuthorityProfile`, bundle ou PDF.

**CORTE 3 E NEXT-05 CONCLUÍDOS EM 12 DE AGOSTO DE 2026.** O `VerificationBundleService` existente empacota o Dossier de `MARKET_TEST_A` sem formato ou manifesto de mercado paralelo. O teste exporta, reconstrói e verifica o pacote offline com trust material sintético, confirmando integridade, assinatura, finalidade e isolamento de `MARKET_TEST_A` contra `MARKET_TEST_B`. O PDF não foi alterado: `LivestockPdfTemplate` atual é uma apresentação farmacológica e não contém os campos de Market Eligibility; reutilizá-lo para este documento esconderia o contexto de mercado. O Dossier/Bundle canônicos permanecem a entrega verificável deste incremento; a apresentação de mercado exige corte próprio caso se torne necessária.
