...
O quarto corte, no commit `69230ba`, aceita a ADR-0056 e integra `MedicationSanitaryClassificationAssertion` à Policy fictícia. `medication_classification` permanece dimensão independente de `treatment_history`; `NO_ASSERTION`, `UNKNOWN` e `DOES_NOT_APPLY` não são equivalentes; conhecimento posterior não entra em seleção anterior ao `knowledge_cutoff`. A migration `20260812_0065`, a API e o roteiro `apps/validacao/classificacao_sanitaria_medicamento.py` fecham o caminho operacional controlado.

Com isso, o NEXT-01 está concluído para `SANITARY_TEST_A_v1`. O próximo portão é elaborar o Design Package do NEXT-02, sem implementação, para provar duas versões fictícias de Policy, seleção por `reference_time`/`knowledge_cutoff` e `NormativeBasisSnapshot` persistido. `NEXT-03` e `NEXT-05` permanecem fora do escopo.
