...
`TITAN_DATABASE_URL` não está configurada, o ambiente não tem banco local, e a
suíte inteira é pulada em vez de falhar por conexão recusada. Gates completos
podem definir `TITAN_REQUIRE_INTEGRATION_DB=1`; nesse modo, ausência de
`TITAN_DATABASE_URL` é erro de configuração, não skip.
...
_REQUIRE_INTEGRATION_DATABASE_ENVIRONMENT_VARIABLE = "TITAN_REQUIRE_INTEGRATION_DB"
...
def pytest_collection_modifyitems(config: pytest.Config, items: list[pytest.Item]) -> None:
    ...
    if os.environ.get(_REQUIRE_INTEGRATION_DATABASE_ENVIRONMENT_VARIABLE) == "1":
