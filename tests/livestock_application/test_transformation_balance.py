"""Balanço mínimo da transformação (ADR-0046, Passo 11.4).
...
são comparadas numericamente (`INDETERMINATE`); e `declared_loss` (perda
conhecida) nunca se confunde com `unaccounted_quantity` (diferença ainda não
explicada) — a segunda é sempre calculada descontando a primeira.
"""
...
def test_saida_sem_quantidade_produz_indeterminate_sem_inventar_zero() -> None:
    balanco = compute_transformation_balance(
        input_quantity=Decimal("200"),
        input_unit="kg",
        input_measurement_basis="peso vivo",
